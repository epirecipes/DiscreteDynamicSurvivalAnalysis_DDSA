...
β_C1_m   = mean(vec(chain[:beta_C1]))
β_C2_m   = mean(vec(chain[:beta_C2]))
β_H_m    = mean(vec(chain[:beta_H]))
β_G_m    = mean(vec(chain[:beta_G]))
mδ_v     = vec(chain[:mean_δ])
nr_vi    = vec(chain[:nb_r])
np_v     = nr_vi ./ (mδ_v .- δ_floor_ip .+ nr_vi)
nb_r_m   = round(Int, mean(nr_vi))
nb_p_m   = mean(np_v)
mean_ip_m = mean(mδ_v)
N0_m     = round(Int, mean(vec(chain[:N0])))
N1_m     = round(Int, mean(vec(chain[:N1])))
N2_m     = round(Int, mean(vec(chain[:N2])))
...
println("β_C1  = $(round(β_C1_m,  digits=4))  (within-class 1)")
println("β_C2  = $(round(β_C2_m,  digits=4))  (within-class 2)")
println("β_H   = $(round(β_H_m,   digits=4))  (household: school → class 0)")
println("β_G   = $(round(β_G_m,   digits=4))  (global uniform mixing)")
...
println("N0 = $N0_m (K0=$K0, observed K0/N0 at posterior-mean N0 = $(round(K0/N0_m,digits=3)))" *
        "  prior: [$K0, $N0_max]")
println("N1 = $N1_m (K1=$K1, observed K1/N1 at posterior-mean N1 = $(round(K1/N1_m,digits=3)))" *
        "  prior: [$K1, $N1_max]")
println("N2 = $N2_m (K2=$K2, observed K2/N2 at posterior-mean N2 = $(round(K2/N2_m,digits=3)))" *
        "  prior: [$K2, $N2_max]")
println()
...
bc1_bc     = mean(vec(chain_bc[:beta_C1]))
bc2_bc     = mean(vec(chain_bc[:beta_C2]))
bh_bc      = mean(vec(chain_bc[:beta_H]))
bg_bc      = mean(vec(chain_bc[:beta_G]))
mδ_bc_v    = vec(chain_bc[:mean_δ])
nr_bc_v    = vec(chain_bc[:nb_r])
np_bc_v    = nr_bc_v ./ (mδ_bc_v .- δ_floor_ip .+ nr_bc_v)
mean_ip_bc = mean(mδ_bc_v)
N0_bc      = round(Int, mean(vec(chain_bc[:N0])))
N1_bc      = round(Int, mean(vec(chain_bc[:N1])))
N2_bc      = round(Int, mean(vec(chain_bc[:N2])))
...
println("nb_r  = $(round(Int, mean(nr_bc_v))),  " *
        "nb_p = $(round(mean(np_bc_v), digits=3)),  " *
        "mean δ = $(round(mean_ip_bc, digits=2)) d")
println("N0 = $N0_bc  (K0=$K0, observed K0/N0 at posterior-mean N0 = $(round(K0/N0_bc,digits=3)))")
println("N1 = $N1_bc  (K1=$K1, observed K1/N1 at posterior-mean N1 = $(round(K1/N1_bc,digits=3)))")
println("N2 = $N2_bc  (K2=$K2, observed K2/N2 at posterior-mean N2 = $(round(K2/N2_bc,digits=3)))")
```
...
R0_glct_mean = round(mean(R0_samples), digits=3)
R0_glct_lo   = round(quantile(R0_samples, 0.025), digits=3)
R0_glct_hi   = round(quantile(R0_samples, 0.975), digits=3)

R0_box_mean  = round(mean(R0_bc_v), digits=3)
R0_box_lo    = round(quantile(R0_bc_v, 0.025), digits=3)
R0_box_hi    = round(quantile(R0_bc_v, 0.975), digits=3)

R0_mean_diff = round(abs(R0_glct_mean - R0_box_mean), digits=3)

println("  GLCT:   mean = $(R0_glct_mean)  95% CI = ($(R0_glct_lo), $(R0_glct_hi))")
println("  Boxcar: mean = $(R0_box_mean)  95% CI = ($(R0_box_lo), $(R0_box_hi))")
```
...
for (label, K, N_m, post, sym) in [
    ...
    N_ci_lo = round(Int, q025(vec(chain[sym])))
    N_ci_hi = round(Int, q975(vec(chain[sym])))
    println("$label:  K=$K,  N=$(N_m) [$(N_ci_lo), $(N_ci_hi)]  " *
            "observed K/N at posterior-mean N=$(round(K/N_m,digits=3))  " *
            "model-implied posterior τ=$(round(mean(post),digits=3))  " *
