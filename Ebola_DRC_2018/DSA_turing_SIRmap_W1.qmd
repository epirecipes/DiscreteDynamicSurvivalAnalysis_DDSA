...
R0_post = β_post / pγ_post   # β/pγ ≈ β/γ_Vossler; consistent with discrete map
mean_ip = 1 / pγ_post        # 1-indexed Geometric mean = 1/pγ ≈ 1/γ_Vossler
println("Parameter Estimates — Wave 1 (all records, posterior means with 95% CI):")
println("β:      $(round(β_post, digits=3)) [$(round(ci_β[1],digits=3)), $(round(ci_β[2],digits=3))]  (Vossler et al.: 0.190)")
println("γ:      $(round(γ_post, digits=3)) [$(round(ci_γ[1],digits=3)), $(round(ci_γ[2],digits=3))]  (Vossler et al.: 0.169)")
println("pγ=1-e⁻ᵞ: $(round(pγ_post, digits=3))")
println("ρ:      $(round(ρ_post, sigdigits=2)) [$(round(ci_ρ[1],sigdigits=2)), $(round(ci_ρ[2],sigdigits=2))]  (Vossler et al.: 0.00021)")
...
println("R₀=β/pγ: $(round(R0_post, digits=3))  (Vossler et al. β/γ: 1.124)")
println("mean IP=1/pγ: $(round(mean_ip, digits=2)) days  (Vossler et al. 1/γ: 5.9 d)")
```
