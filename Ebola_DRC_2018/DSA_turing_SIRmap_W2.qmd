...
mean_ip = 1 / pγ_post
println("Parameter Estimates — Wave 2 (all records, posterior means with 95% CI):")
println("β:      $(round(β_post, digits=3)) [$(round(ci_β[1],digits=3)), $(round(ci_β[2],digits=3))]  (Vossler: 0.217)")
println("γ:      $(round(γ_post, digits=3)) [$(round(ci_γ[1],digits=3)), $(round(ci_γ[2],digits=3))]  (Vossler: 0.179)")
println("pγ=1-e⁻ᵞ: $(round(pγ_post, digits=3))")
println("ρ:      $(round(ρ_post, sigdigits=2)) [$(round(ci_ρ[1],sigdigits=2)), $(round(ci_ρ[2],sigdigits=2))]  (Vossler: 0.0054)")
...
println("R₀=β/pγ: $(round(R0_post, digits=3))  (Vossler β/γ: 1.214)")
println("mean IP=1/pγ: $(round(mean_ip, digits=2)) days  (Vossler 1/γ: 5.6 d)")
```
