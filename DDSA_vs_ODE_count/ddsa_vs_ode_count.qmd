...
This notebook contrasts discrete dynamic survival analysis (DDSA) with the familiar workflow of fitting a deterministic compartmental ODE to aggregated daily counts under a Poisson or Negative-Binomial observation model (see the [epirecipes notebook](https://github.com/epirecipes/sir-julia/blob/master/markdown/ode_turing/ode_turing.md)).
...
The count likelihood and DDSA align more closely than it first appears. Independent Poisson daily counts, conditioned on their total $K$, are exactly $\mathrm{Multinomial}(K,f)$, just like DDSA's infection-time term. The count--Poisson log-likelihood therefore factorises as
...
DDSA keeps the multinomial timing term, swaps the Poisson final-size factor for $\mathrm{Binomial}(N,\tau)$—the natural counterpart when the at-risk population $N$ is known—and adds a recovery-interval term that aggregated counts cannot represent. Under Poisson noise, a correctly specified infectious period, and large $N$, the two approaches agree on infection timing. They diverge where the line list carries extra information: individual recovery durations (identifying IP mean and shape) and a tighter small-$N$ final-size constraint. For the ODE comparator, $\mu_t$ comes from a continuous solve sampled at integer days, so its timing PMF $f$ matches DDSA's only up to discretisation. The chunk below verifies the factorisation on the exponential ODE's mean curve at the generative truth:
...
For the three $\gamma$-free count models we use high target-accept / deep-tree NUTS settings to clear divergences on the weak $\beta$--$\gamma$ ridge, so that wide posteriors reflect weak identifiability rather than sampler failure. The $\gamma$-fixed model is well behaved and uses milder NUTS. DDSA mixes discrete shape $r$ (Metropolis--Hastings) with NUTS on $(\beta,\rho,p)$.
...
The section below displays convergence diagnostics ($\hat R$, ESS, divergences) and posterior medians with 95% credible intervals for $\beta$, mean IP, and $R_0$ by model.
...
In (B), $\gamma$-free count posteriors (blue, purple) stay broad — near or overlapping the prior — reflecting the unresolved $\beta$--$\gamma$ ridge. The $\gamma$-fixed ODE (green) narrows the posterior but is biased downward: fixing $\gamma$ at the assumed continuous-time mean ($\mu=4$ d) overestimates the removal rate relative to the effective discrete IP, so the inferred $R_0 = \hat\beta/\gamma_{\mathrm{fix}}$ undershoots the truth. Only DDSA (red) concentrates near the truth. In (C), only DDSA recovers the mean IP from data (model 4 not shown as it has no IP posterior).
...
- Models 1--2 (Poisson and NegBin, $\gamma$ free): $R_0$ posteriors in panel (B) are broad and overlap the prior, reflecting weak joint identifiability of $\beta$ and $\gamma$ from counts alone. The mean IP posterior in panel (C) is equally diffuse. 
- Model 3 (staged ODE, $\gamma$ free): a practitioner who suspects a non-exponential IP and uses the correctly staged model still cannot identify the IP mean, the purple curve in panel (B) remains diffuse.
- Model 4 (exponential, $\gamma$ fixed from literature): anchoring $\gamma$ at the assumed continuous-time mean IP of 4 d narrows the $R_0$ posterior, but the 95\% CrI lies below truth. Fixing $\gamma = 1/4$ overestimates the removal rate relative to the effective discrete IP of 6.33 d, so the inferred $R_0 = \hat\beta / \gamma_{\mathrm{fix}}$ is too small even as $\hat\beta$ rises to fit the epidemic curve. This bias would persist undetected without access to individual recovery times.
